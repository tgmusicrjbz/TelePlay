...
function PlaylistIndex() {
    ...
    return <div className="mx-auto w-full max-w-7xl p-4 pb-28 sm:p-6 md:pb-8 lg:p-8">
        ...
        <section className="mt-8"><h2 className="mb-3 text-lg font-bold">پلی‌لیست‌های هوشمند ✨</h2><div className="flex snap-x gap-3 overflow-x-auto pb-2">{smart.map(item => <button key={item.title} disabled={!item.files.length} onClick={() => startQueue(item.files, 0)} className="min-w-[220px] snap-start rounded-2xl border border-white/[.07] bg-dark-900/70 p-3 text-right disabled:opacity-55"><span className="block truncate font-bold">{item.title}</span><span className="mt-2 block truncate text-xs text-dark-400">{item.files.length.toLocaleString('fa-IR')} مورد · {item.subtitle}</span></button>)}</div></section>
        <section className="mt-8"><div className="mb-3 flex flex-wrap items-center gap-2"><ListMusic className="h-5 w-5 text-primary-300"/><h2 className="text-lg font-bold">ساخته‌های من</h2><div className="mr-auto flex items-center gap-1 rounded-xl border border-white/10 bg-dark-900/70 p-1"><select aria-label="مرتب‌سازی پلی‌لیست‌ها" value={sort} onChange={e => setSort(e.target.value as typeof sort)} className="h-8 rounded-lg bg-transparent px-2 text-xs text-dark-200 outline-none"><option value="updated">تازه‌ترین</option><option value="name">نام</option><option value="count">تعداد ترک</option></select><button aria-label="نمای کارتی" title="نمای کارتی" onClick={() => setView('grid')} className={`rounded-lg p-1.5 ${view === 'grid' ? 'bg-primary-500/20 text-primary-200' : 'text-dark-400'}`}><LayoutGrid className="h-4 w-4"/></button><button aria-label="نمای لیستی" title="نمای لیستی" onClick={() => setView('list')} className={`rounded-lg p-1.5 ${view === 'list' ? 'bg-primary-500/20 text-primary-200' : 'text-dark-400'}`}><List className="h-4 w-4"/></button></div></div>{isLoading ? <div className="h-36 animate-pulse rounded-2xl bg-dark-800"/> : visible.length ? <div className={view === 'grid' ? 'grid gap-4 sm:grid-cols-2 xl:grid-cols-3' : 'flex flex-col gap-2'}>{visible.map(p => <PlaylistCard key={p.id} playlist={p} view={view} />)}</div> : <div className="rounded-2xl border border-dashed border-white/10 py-12 text-center text-dark-400">هنوز پلی‌لیستی اینجا نیست.</div>}</section>
...
function PlaylistDetail({ id, onBack }: { id: number; onBack: () => void }) {
    ...
    const reorder = useReorderPlaylist(); const shuffle = useShufflePlaylist(); const remove = useRemovePlaylistItem(); const del = useDeletePlaylist(); const upload = useUploadFile(); const update = useUpdatePlaylist();
    const { startQueue, addToast } = useAppStore();
    ...
    const dragEnd = async ({ active, over }: DragEndEvent) => { setActiveItem(null); if (!over || active.id === over.id) return; const old = orderedItems.findIndex(x => x.id === active.id); const next = orderedItems.findIndex(x => x.id === over.id); const nextItems = arrayMove(orderedItems, old, next); setOrderedItems(nextItems); try { await reorder.mutateAsync({ id, itemIds: nextItems.map(x => x.id) }); addToast('ترتیب پلی‌لیست ذخیره شد'); } catch { setOrderedItems(playlist.items); addToast('ذخیرهٔ ترتیب انجام نشد', 'error'); } };
    const uploadCover = async (event: React.ChangeEvent<HTMLInputElement>) => { const file = event.target.files?.[0]; event.target.value = ''; if (!file || !file.type.startsWith('image/')) return; const uploaded = await upload.mutateAsync({ file, folderId: null }); await update.mutateAsync({ id, cover_file_id: uploaded.id }); setCoverPicker(false); addToast('کاور پلی‌لیست تغییر کرد 🖼️'); };
