...
export default function SettingsPage() {
    ...
    return <div className="mx-auto w-full max-w-4xl p-4 pb-28 sm:p-6 md:pb-8 lg:p-8">
        ...
        <div className="grid gap-4 md:grid-cols-2">
            <section className="rounded-2xl border border-white/[0.07] bg-dark-900/70 p-5"><div className="flex items-center gap-3"><HardDrive className="h-5 w-5 text-primary-300" /><h2 className="font-bold">حافظه کمد</h2></div><p className="mt-5 text-3xl font-black">{storage ? formatFileSize(storage.total_size) : '…'}</p><p className="mt-2 text-sm text-dark-400">حجم فایل‌های ذخیره‌شده در تلگرام</p></section>
            <section className="rounded-2xl border border-white/[0.07] bg-dark-900/70 p-5"><div className="flex items-center gap-3"><Smartphone className="h-5 w-5 text-primary-300" /><h2 className="font-bold">حساب تلگرام</h2></div><p className="mt-5 text-lg font-bold">{displayName}</p><p className="mt-1 flex items-center gap-1.5 text-sm text-dark-400" dir="ltr"><AtSign className="h-4 w-4" />{user?.username || 'بدون نام کاربری'}</p><p className="mt-2 text-xs text-dark-500">شناسه: {user?.telegram_id?.toLocaleString('fa-IR') || '…'}</p></section>
