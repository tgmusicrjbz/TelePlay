...
export default function Sidebar() {
    ...
    const selectSection = (section: typeof activeSection) => {
        ...
        const path = section === 'playlists' ? '/playlists' : '/';
    ...
    return <>
        ...
        <nav className="fixed inset-x-0 bottom-0 z-[110] grid h-[calc(5rem+env(safe-area-inset-bottom))] grid-cols-4 border-t border-white/10 bg-dark-900/95 px-1 pb-[env(safe-area-inset-bottom)] backdrop-blur-xl md:hidden">{items.map(({ section, shortLabel, icon: Icon }) => <button key={section} onClick={() => selectSection(section)} className={`flex min-h-14 flex-col items-center justify-center gap-1 text-[11px] transition-colors ${activeSection === section ? 'text-primary-300' : 'text-dark-400'}`}><span className={`flex h-8 w-12 items-center justify-center rounded-full ${activeSection === section ? 'bg-primary-500/15' : ''}`}><Icon className="h-5 w-5" /></span><span>{shortLabel}</span></button>)}</nav>
    </>;
}
